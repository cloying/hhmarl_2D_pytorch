# FILE: train_hetero.py (FINAL - Corrected for Sparse Info and All Previous Bugs)
...
class VectorizedRolloutBuffer:
    def __init__(self, num_steps, num_workers, agent_ids, obs_spaces, act_spaces, device):
        self.num_steps, self.num_workers, self.agent_ids, self.device = num_steps, num_workers, agent_ids, device
        self.obs = {i: torch.zeros((num_steps, num_workers, *obs_spaces[i].shape), device=device) for i in agent_ids}
        self.actions = {i: torch.zeros((num_steps, num_workers, len(act_spaces[i].nvec)), device=device) for i in
                        agent_ids}
    ...
    def add(self, obs, actions, logprobs, rewards, dones, values):
        for agent_id in self.agent_ids:
            if agent_id in obs:
                ...
                self.rewards[agent_id][self.step] = torch.tensor(rewards.get(agent_id, np.zeros(self.num_workers)),
    ...
    def compute_advantages_and_returns(self, next_values, next_dones, gamma, gae_lambda):
        ...
        for agent_id in self.agent_ids:
            ...
            for t in reversed(range(self.num_steps)):
                ...
                next_value = next_values.get(agent_id, torch.zeros(self.num_workers,
                                                                   device=self.device)) if t == self.num_steps - 1 else \
                self.values[agent_id][t + 1]
...
if __name__ == "__main__":
    ...
    buffer = VectorizedRolloutBuffer(num_steps, num_workers, agent_ids, vec_env.single_observation_space,
                                     vec_env.single_action_space, device)
        ...
        for step in range(num_steps):
            with torch.no_grad():
                ...
                for policy_id, model in models.items():
                    ...
                    critic_obs_dict = {'obs_1_own': obs_tensors[main_agent_id], 'act_1_own': actions[main_agent_id],
                                       'obs_2': obs_tensors[other_agent_id], 'act_2': actions[other_agent_id]}
            ...
            # --- FINAL FIX: Defensively check for the rewards dictionary from each worker ---
            if "agent_rewards" in infos:
                agent_rewards_dicts = infos["agent_rewards"]  # This is an array of dicts or Nones
                for i in range(num_workers):
                    worker_rewards_dict = agent_rewards_dicts[i]
                    # Check if the info for this worker is not None
                    if worker_rewards_dict is not None:
        ...
        with torch.no_grad():
            ...
            for policy_id, model in models.items():
                ...
                critic_obs_dict = {'obs_1_own': next_obs_tensors[main_agent_id],
                                   'act_1_own': torch.zeros_like(actions[main_agent_id]),
                                   'obs_2': next_obs_tensors[other_agent_id],
                                   'act_2': torch.zeros_like(actions[other_agent_id])}
                next_val = model.get_value(critic_obs_dict).squeeze()
                for aid, pid in policy_map.items():
                    if pid == policy_id:
                        next_values[aid] = next_val
        buffer.compute_advantages_and_returns(next_values, torch.tensor(dones, dtype=torch.float32, device=device),
                                              gamma, gae_lambda)
        ...
        for epoch in range(num_update_epochs):
            for mini_batch in buffer.get_mini_batches(mini_batch_size):
                for policy_id, model in models.items():
                    ...
                    critic_obs_dict = {'obs_1_own': mini_batch[main_agent_id]['obs'],
                                       'act_1_own': mini_batch[main_agent_id]['actions'],
                                       'obs_2': mini_batch[other_agent_id]['obs'],
                                       'act_2': mini_batch[other_agent_id]['actions']}
                    ...
                    new_logprobs = torch.sum(
                        torch.stack([d.log_prob(a) for d, a in zip(dists, mini_batch[main_agent_id]['actions'].T)]),
                        dim=0)
                    ...
                    pg_loss = torch.max(-advantages * ratio,
                                        -advantages * torch.clamp(ratio, 1 - clip_coef, 1 + clip_coef)).mean()
