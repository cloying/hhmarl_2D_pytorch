# FILE: train_hetero.py (With GIF Generation)
...
import tqdm
import imageio  # <-- NEW IMPORT FOR GIFS
...
def evaluate_and_render_episode(args, policy_map, models, device, writer, global_step):
    """
    Runs one episode in a single environment, saves each frame, and compiles a GIF.
    """
    ...
    # 1. Create a single, dedicated environment for rendering
    ...
    while not done:
        ...
        # 4. Render the frame and store it
    ...
    imageio.mimsave(gif_path, frames, duration=100)  # duration is in ms per frame
...
if __name__ == "__main__":
    ...
    # --- NEW: Visualization Frequency ---
    render_every_n_updates = 50
    ...
    # --- 3. Agent and Policy Initialization ---
    ...
    for update in pbar:
        ...
        for step in range(num_steps):
            ...
            with torch.no_grad():
                ...
                for policy_id, model in models.items():
                    ...
                    critic_obs_dict = {
                        'obs_1_own': obs_tensors[main_agent_id], 'act_1_own': actions[main_agent_id].float(),
            ...
            for i in range(num_workers):
                ...
                if worker_rewards:
                    for agent_id in agent_ids:
                        reward_buffers[agent_id][step, i] = worker_rewards.get(agent_id, 0.0)
        ...
        with torch.no_grad():
            ...
            for policy_id, model in models.items():
                ...
                critic_obs_dict = {
                    'obs_1_own': next_obs_tensors[main_agent_id], 'act_1_own': dummy_actions[main_agent_id],
                    'obs_2': next_obs_tensors[other_agent_id], 'act_2': dummy_actions[other_agent_id]
                }
        ...
        for epoch in range(num_update_epochs):
            ...
            for start in range(0, batch_size, mini_batch_size):
                ...
                for policy_id, model in models.items():
                    ...
                    critic_obs_dict = {
                        'obs_1_own': mb_obs_own, 'act_1_own': mb_act_own.float(),
                        'obs_2': mb_obs_other, 'act_2': mb_act_other.float()
                    }
        ...
        ### --- NEW: Call the rendering function periodically --- ###
        if update > 0 and update % render_every_n_updates == 0:
            # Set models to evaluation mode for deterministic actions
    ...
    print(f"\nFinal models saved to '{policy_dir}' directory.")
