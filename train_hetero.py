# FILE: train_hetero.py (Final Version with All Features)
...
def load_checkpoint(actors, critics, actor_optimizers, critic_optimizers, path, device):
...
if __name__ == "__main__":
    ...
    if args.restore:
        learning_rate = 3e-5
        print(f"Restore mode enabled. Using smaller learning rate for fine-tuning: {learning_rate}")
    ...
    for agent_id in agent_ids:
        is_ac1_type = (agent_id % 2) != 0  # Agents 1, 3 are AC1 type
        ...
        if policy_id not in actors:
            ...
            own_id = 1 if is_ac1_type else 2  # Use canonical IDs for spaces

            print(f"Creating models for {policy_id} (Aircraft Type {own_id})")
            ...
            # Check if PyTorch version is 2.0+ before compiling
            if torch.__version__ >= "2.0.0":
                print(f"Compiling models with torch.compile() for {policy_id}")
                actor = torch.compile(actor)
                critic = torch.compile(critic)

            actors[policy_id] = actor
            critics[policy_id] = critic
            actor_optimizers[policy_id] = optim.Adam(actor.parameters(), lr=learning_rate, eps=1e-5)
            critic_optimizers[policy_id] = optim.Adam(critic.parameters(), lr=learning_rate, eps=1e-5)
    ...
    if args.restore and args.restore_path:
        ...
        if latest_checkpoint:
            ...
            print(f"Warning: Restore set to True, but no checkpoints found in {args.restore_path}")
    ...
    for update in pbar:
        for step in range(num_steps):
            ...
            with torch.no_grad():
                ...
                for policy_id in actors.keys():
                    value = critics[policy_id](graph_batch) if graph_batch else torch.zeros(num_workers, device=device)
        ...
        for epoch in range(num_update_epochs):
            ...
            for start in range(0, batch_size, mini_batch_size):
                ...
                for policy_id in actors.keys():
                    ...
                    # Note: We update actor and critic separately
                    # Actor Loss
