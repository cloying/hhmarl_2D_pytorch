# FILE: config.py (Final Vetted Version with Tunable Rewards)
...
class Config(object):
    def __init__(self, mode: int):
        ...
        ### --- NEW: Tunable Reward Magnitudes --- ###
        parser.add_argument('--kill_reward_bonus', type=float, default=2.0,
                            help='Additional sparse reward for destroying an enemy.')
        parser.add_argument('--firing_reward', type=float, default=0.02,
                            help='Dense reward for firing under good conditions.')
        ...
        parser.add_argument('--shaping_scale_initial', type=float, default=0.01,
                            help='Initial multiplier for dense shaping rewards.')
        parser.add_argument('--shaping_scale_final', type=float, default=0.0,
                            help='Final multiplier for shaping rewards.')
        parser.add_argument('--shaping_decay_timesteps', type=int, default=None,
                            help='Timesteps until scale reaches final value. (Default: 50% of total_timesteps)')

        # --- Evaluation-Specific Parameters ---
        parser.add_argument('--eval_info', type=bool, default=True if mode == 2 else False,
                            help='Provide eval statistic in step() function or not.')
    ...
    def set_metrics(self):
        """Sets up logging paths, restore logic, and dynamic hyperparameters."""
        if self.mode == 0:
