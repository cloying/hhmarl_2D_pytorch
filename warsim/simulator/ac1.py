...
import numpy as np
# This file should NOT import 'random'.
...
class Rafale(Unit):
    # ... (class attributes are unchanged) ...
    max_deg_sec = 5; min_speed_knots = 0; max_speed_knots = 900; max_knots_sec = 35;
    cannon_range_km = 2.0; cannon_width_deg = 10; cannon_max_time_sec = 200;
    cannon_burst_time_sec = 5; cannon_hit_prob = 0.75; max_missiles = 5;
    missile_range_km = 111; missile_width_deg = 120; aircraft_type = 1
    ...
    # ... (set_heading, set_speed, fire_cannon, fire_missile are unchanged) ...
    ...
    def fire_cannon(self): self.cannon_current_burst_secs = min(self.cannon_remain_secs, Rafale.cannon_burst_time_sec)
    def fire_missile(self, ag_unit:Unit, opp_unit:Unit, sim: CmanoSimulator):
        if not self.actual_missile and self.missile_remain > 0:
            if units_distance_km(self, opp_unit) <= Rafale.missile_range_km and self._angle_in_radar_range(units_bearing(self, opp_unit)):
    ...
    def update(self, tick_secs: float, sim: CmanoSimulator) -> List[Event]:
                ...
                if unit.id != self.id and unit.id <= (sim.num_units + sim.num_opp_units):
                    ...
                    if self.friendly_check or is_opponent:
                        if unit.type in ["RafaleLong", "Rafale"] and self._unit_in_cannon_range(unit):
                            # --- MODIFICATION: Use the simulator's safe random generator ---
                            # OLD: if sim.rnd_gen.random() < ...
                            if sim.random_generator.random() < (Rafale.cannon_hit_prob / (Rafale.cannon_burst_time_sec / tick_secs)):
        ...
        if self.actual_missile:
            if not sim.unit_exists(self.actual_missile.id):
                ...
                # --- MODIFICATION: Use the simulator's safe random generator ---
                # OLD: heading = np.clip(self.actual_missile.heading * random.uniform(0.95, 1.05), 0, 359)
    ...
    # ... (_unit_in_cannon_range and _angle_in_radar_range are unchanged) ...
